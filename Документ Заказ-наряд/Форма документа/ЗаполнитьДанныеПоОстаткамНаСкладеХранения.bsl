...
Процедура ЗаполнитьДанныеПоОстаткамНаСкладеХранения()
	...
	//               |ГДЕ
	//               |	СтруктурныеЕдиницыДополнительныеРеквизиты.Свойство.Наименование ПОДОБНО ""Код подразделения%""
	//               |	И (ВЫРАЗИТЬ(СтруктурныеЕдиницыДополнительныеРеквизиты.Значение КАК СТРОКА(9))) = &КодПодразделения
	//               |;
	//               |
	//               |////////////////////////////////////////////////////////////////////////////////
	//               |ВЫБРАТЬ
	...
	//               |		RMS_РезервыПодЗаказНарядОстатки.СтруктурнаяЕдиница,
	//               |		0,
	...
	//               |	ВЫБОР
	//               |		КОГДА ВТ_Данные.КоличествоОстаток > ВТ_Данные.КоличествоРезерв
	...
	Пока Выборка.Следующий() Цикл
		...
		// +++ Инфостарт #2507 20.08.2021 Распределение резерва по остаткам
		...
		//	Если ВыборкаДетальныеЗаписи.Распределять Тогда
		//		
		//		Коэффициенты.Добавить(ВыборкаДетальныеЗаписи.КоличествоВНаличии);
		//		СуммаРаспределения = СуммаРаспределения + ВыборкаДетальныеЗаписи.КоличествоРезерв;
		//		
		//	Иначе
		//		
		...
		Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
			...
			//Иначе
			//	
			//	Если ВыборкаДетальныеЗаписи.КоличествоВНаличии - РезультатРаспределения[Сч] > 0 Тогда
			//		МассивСкладовВНаличии.Добавить(Строка(ВыборкаДетальныеЗаписи.Склад) + " - " + Формат(ВыборкаДетальныеЗаписи.КоличествоВНаличии - РезультатРаспределения[Сч], "ЧГ=0"));
			//	КонецЕсли;
			//	
			//	Сч = Сч + 1;
			//	
	...
	// +++ Инфостарт #2507 20.08.2021 Временно закомментировано из-за пересечения задач
	//// +++ Инфостарт #2666 13.08.2021 Заполнение актуальных резервов только по текущему заказ-наряду
	//Для Каждого СтрокаДавалСырья Из Объект.МатериалыЗаказчика Цикл
	//	СтрокаДавалСырья.дм_РезервНаСкладах ="";
	//КонецЦикла;
	//
	...
	//		//|	RMS_РезервыПодЗаказНарядОстатки.КоличествоОстаток КАК КоличествоРезерв
	//		|	ВЫБОР
	...
	//		|	СУММА(RMS_РезервыПодЗаказНарядОстатки.КоличествоОстаток) КАК КоличествоРезерв
	//		// --- Инфостарт #2507 20.08.2021
	//		|ИЗ
	//		|	РегистрНакопления.RMS_РезервыПодЗаказНаряд.Остатки(
	//		|			&Дата,
	...
	//		|					ИЗ
	//		|						ВТМатериалы КАК ВТМатериалы)) КАК RMS_РезервыПодЗаказНарядОстатки
	//		|ГДЕ
	//		|	RMS_РезервыПодЗаказНарядОстатки.КоличествоОстаток > 0
	//		// +++ Инфостарт #2507 20.08.2021
	//		|
	...
	//	Запрос.УстановитьПараметр("Дата",              ТекущаяДата());
	//	Запрос.УстановитьПараметр("ЗаказНаряд",        Объект.Ссылка);
	//	Запрос.УстановитьПараметр("ТаблицаМатериалов", Объект.МатериалыЗаказчика.Выгрузить(, "Номенклатура, Характеристика"));
	//	
	//	РезультатЗапроса = Запрос.Выполнить();
	//	
	...
	//		
	//		ВыборкаХарактеристик = ВыборкаТовара.Выбрать(ОбходРезультатаЗапроса.ПоГруппировкам);
	//		
	//		Пока ВыборкаХарактеристик.Следующий() Цикл
	//			
	//			Выборка = ВыборкаХарактеристик.Выбрать();
	//			
	//			МассивСкладовРезервов = Новый Массив;
	//			
	//			Пока Выборка.Следующий() Цикл
	//				МассивСкладовРезервов.Добавить(Строка(Выборка.СтруктурнаяЕдиница) + " - " + Формат(Выборка.КоличествоРезерв, "ЧГ=0"));
	//			КонецЦикла;
	//			
	//			ОтборМатериалов = Новый Структура;
	//			ОтборМатериалов.Вставить("Номенклатура",   ВыборкаТовара.Номенклатура);
	//			ОтборМатериалов.Вставить("Характеристика", ВыборкаХарактеристик.Характеристика);
	//			
	//			НайденныеСтрокиМатериалов = Объект.МатериалыЗаказчика.НайтиСтроки(ОтборМатериалов);
	//			
	//			Для Каждого СтрокаМатериалов Из НайденныеСтрокиМатериалов Цикл
	//				СтрокаМатериалов.дм_РезервНаСкладах = СтрСоединить(МассивСкладовРезервов, "/");
	//			КонецЦикла;
	//			
	//		КонецЦикла;
	//		
